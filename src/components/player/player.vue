  ...
  export default {
    data() {
      return {
        ...
        currentTime: 0,
        radius: 32,
        currentLyric: null, // 当前歌词 是一个Lyric class
        currentLineNum: 0, // 歌词的行数
        playingLyric: '',
        currentShow: 'cd'
      }
    },
    ...
    methods: {
      ...
      leave(el, done) {
        ...
        const {x, y, scale} = this._getPosAndScale()
        console.log('x: "' + this._getPosAndScale().x + 'y:' + this._getPosAndScale().y + 'z:' + this._getPosAndScale().scale)
      ...
      middleTouchMove(e) {
        ...
        console.log(deltaX + ':' + Math.max(-window.innerWidth, left + deltaX))
      ...
      middleTouchEnd(e) {
        ...
        if (this.currentShow === 'cd') {
          if (this.touch.percent > 0.1) {
            ...
            opacity = 0
            this.$refs.lyricList.$el.style[transitionDuration] = `translate3d(${-window.innerWidth}px,0,0)`
      ...
      handleLyric({lineNum, txt}) {
        ...
        console.log('this.currentLineNum: ', this.currentLineNum)
        if (lineNum > 5) {
